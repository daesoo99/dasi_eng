/**
 * Level 4 Curriculum Validation Test
 * 테스트용 스크립트 - L4 REVISED 검증
 */

const fs = require('fs');
const path = require('path');

// Load the Level 4 REVISED curriculum
function loadCurriculum() {
  const filePath = path.join(__dirname, 'patterns/level_4_advanced_expressions/lv4_stage_system_REVISED.json');
...
function validateStageIds(stages) {
  const issues = [];
  const expectedIds = [
    // Phase 1: Advanced Tenses (5 stages)
    'Lv4-P1-S01', 'Lv4-P1-S02', 'Lv4-P1-S03', 'Lv4-P1-S04', 'Lv4-P1-S05',
    // Phase 2: Complex Sentence Structures (5 stages)
    'Lv4-P2-S06', 'Lv4-P2-S07', 'Lv4-P2-S08', 'Lv4-P2-S09', 'Lv4-P2-S10',
    // Phase 3: Clause Transformations (5 stages)
    'Lv4-P3-S11', 'Lv4-P3-S12', 'Lv4-P3-S13', 'Lv4-P3-S14', 'Lv4-P3-S15',
    // Phase 4: Formality Control (5 stages)
    'Lv4-P4-S16', 'Lv4-P4-S17', 'Lv4-P4-S18', 'Lv4-P4-S19', 'Lv4-P4-S20',
    // Phase 5: Advanced Expressions (4 stages)
    'Lv4-P5-S21', 'Lv4-P5-S22', 'Lv4-P5-S23', 'Lv4-P5-S24',
    // Phase 6: Logical Communication (5 stages)
    'Lv4-P6-S25', 'Lv4-P6-S26', 'Lv4-P6-S27', 'Lv4-P6-S28', 'Lv4-P6-S29'
  ];
...
function validateGrammarProgression(stages) {
  ...
  const expectedTopics = [
    'future perfect', 'causative', 'inversion', 'subjunctive', 'participle',
    'reduced relative', 'appositives', 'hedging', 'discourse markers',
    'formal register', 'nominalization', 'abstract', 'logical cohesion'
  ];
  
  const titles = stages.map(s => s.title).join(' ');
  const descriptions = stages.map(s => s.description).join(' ');
  const combinedText = (titles + ' ' + descriptions).toLowerCase();
  
  expectedTopics.forEach(topic => {
    if (!combinedText.includes(topic.toLowerCase())) {
      issues.push(`WARNING: Grammar topic "${topic}" not clearly covered in stage content`);
    }
  });
...
function validatePhases(data) {
  ...
  const expectedPhaseStructure = {
    ...
    2: { title: '복합 문장', expectedStages: 5 },
    3: { title: '절 변환', expectedStages: 5 },
    4: { title: '격식성', expectedStages: 5 },
    5: { title: '고급 표현', expectedStages: 4 },
    6: { title: '논리적 소통', expectedStages: 5 }
  };
...
function runValidation() {
  console.log('=== Level 4 REVISED Curriculum Validation ===\n');
  ...
  try {
    ...
    if (errors.length === 0 && warnings.length === 0) {
      console.log('✅ ALL TESTS PASSED! Level 4 REVISED is ready for deployment.');
    } else if (errors.length === 0) {
      console.log('✅ VALIDATION PASSED with minor warnings. Level 4 REVISED is ready for deployment.');
    }
